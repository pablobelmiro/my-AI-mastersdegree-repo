...
Este relatório descreve a **Parte 2** do trabalho, onde o desafio foi sair da teoria das linguagens regulares e construir um motor capaz de reconhecer padrões de texto usando autômatos.

## 1. O Problema Proposto

O objetivo era desenvolver um motor de busca ou gerador de autômatos baseado em expressões regulares. Para isso, foi proposto a utilização da metodologia de **Construção de Thompson**. Essa metodologia permite "transformar" qualquer expressão regular em um NFA-$\epsilon$ equivalente. 
...
### Estrela de Kleene (`a*`)
Cria-se um loop que permite voltar ao início para repetir a letra ou saltar direto para o fim em casos de zero repetições.
```mermaid
graph LR
    0((0)) -- epsilon --> 1((1))
    0((0)) -- epsilon --> 3(((3)))
...
## 4. O Motor de Reconhecimento

Para testar se uma palavra pertence à linguagem da Regex, o simulador realiza um **rastreio de estados simultâneos**:

1.  **Estado Atual:** Começamos no estado inicial e calculamos seu **Fecho-Epsilon** que são todos os estados onde a máquina pode estar "de graça".
2.  **Consumo de Símbolos:** Para cada letra lida, a máquina realiza a transição para todos os estados possíveis e, imediatamente, calcula o novo Fecho-Epsilon.
...
1.  Começamos com uma **Expressão Regular**.
2.  Transformamos em um **NFA-$\epsilon$** utilizando a metodologia de Thompson.
